...
def main():
    ...
    with open(workdir / f"{dataname}TC.input.txt", "w") as f:
    ...
    with open(workdir / f"{dataname}TC.DNoutput.txt", "w") as f:
        for date in common_dates:
            f.write(f"{workdir}/{dataname}_{date_str(date)}_TC.txt\n")

    print(f"\n[DONE] {dataname} TC: {len(common_dates)} time steps")
